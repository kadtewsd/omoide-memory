...
sealed interface UploadPoint : EnumWithLabel {
    ...
    /**
     * 総件数 (totalRequestCount) を考慮した次のステップポイント。
     * 通常は [next] を返すが、Start など件数によって遷移先が分岐するステップはオーバーライドする。
     */
    fun next(totalRequestCount: Int): UploadPoint = next
    ...
    data object Start : Doing {
        ...
        override fun next(totalRequestCount: Int): UploadPoint = if (totalRequestCount == 0) TargetEmpty else WorkerStarted
    }
...
class UploadReport(
    ...
    val id: Long,
    val createdAt: Long,
    lastPoint: UploadPoint,
    totalRequestCount: Int,
    successCount: Int,
    updatedAt: Long,
    message: String?,
) {
    var lastPoint: UploadPoint = lastPoint
        private set

    var totalRequestCount: Int = totalRequestCount
        private set
    ...
    /**
     * 処理済み件数 (processedCount) を受け取り、次のステップへ進行します。
     * 契約による設計に基づき、自身の内部状態 (totalRequestCount) を元に
     * 次のステップへ自己完結して遷移し、メッセージを反映します。
     */
    fun next(processedCount: Int): UploadReport =
        also {
            this.successCount = processedCount
            this.updatedAt = System.currentTimeMillis()
            this.lastPoint = this.lastPoint.next(this.totalRequestCount)
            this.message = this.lastPoint.message
        }
