...
**ルール**:
- 分岐・パターン・状態が発生する場合は `sealed interface` / `sealed class` などの **ADT（代数的データ型）** を定義
- `if` や OOP の継承は使わず、`when` 式や `is` による網羅的なパターンマッチで分岐・状態を表現
- 何が起きうるかを型で明示
- **【厳格遵守】状態判断や条件判定をローカルの Boolean フラグ変数（`val isProcessing = ...`, `val isSuccess = ...` 等）に言い換えることを禁止する**。判定が必要な箇所で直接 ADT の `when` 式または `is` 式で表現すること。フラグで言い換えることは、冗長に直接判定を書くことよりも更に悪である。
