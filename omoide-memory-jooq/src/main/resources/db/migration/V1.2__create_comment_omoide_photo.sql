...
CREATE TABLE comment_omoide_photo (
    ...
    CONSTRAINT fk_comment_omoide_photo_photo FOREIGN KEY (file_name) REFERENCES synced_omoide_photo (file_name) ON DELETE CASCADE,
    CONSTRAINT uq_comment_omoide_photo_seq UNIQUE (file_name, comment_seq)
);

COMMENT ON TABLE comment_omoide_photo IS '写真に対するコメント';
COMMENT ON COLUMN comment_omoide_photo.id IS 'コメントID（サロゲートキー）';
COMMENT ON COLUMN comment_omoide_photo.file_name IS '対象ファイル名（外部キー）';
